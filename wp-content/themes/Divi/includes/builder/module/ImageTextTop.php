...
class ET_Builder_Module_ImageTextTop extends ET_Builder_Module {
	function init() {
		$this->name       = 'Image text top';
		$this->slug       = 'et_pb_image_text_top';
		$this->vb_support = 'on';
	...
	function render( $attrs, $content = null, $render_slug ) {
		...
		$output = sprintf(
			'<div class="et_pb_image_wrap image-text-top"><img src="%1$s" alt="%2$s"%3$s />%4$s' . '<div class="text-img">' . $title_text . '</div></div>',
			esc_attr( $src ),
			esc_attr( $alt ),
...
function _et_bb_module_image_add_src_label1( $filed ) {
...
add_filter( 'et_builder_module_fields_et_pb_image_field_src', '_et_bb_module_image_add_src_label1' );

new ET_Builder_Module_ImageTextTop;
